...
//                        UIImageWriteToSavedPhotosAlbum(decodeImage!,self,nil,nil)   // 端末に画像を保存
    ...
    func registerUserStateToRooms(roomId: String, uid: String, state: String, name: String, iconName: String) -> Observable<Bool> {
        ...
        return Observable<Bool>.create { observer in
            ...
            roomsRef.collection("UsersStates").document(uid).setData(["state": state, "name": name, "iconName": iconName]) { err in
                if let err = err {
